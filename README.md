 ...
 * If I already have a username, I will use that information to log in by making a POST request to http://localhost:3000/api/v1/login?username=ellishim&password=12345. A successful response body will return a json object like mentioned in the previous bullet point, but the user object will also contain an array of urls that belong to the user. Those are returned as a result of the `UserSerializer`.
...
* I can view all my created urls by making a GET request to http://localhost:3000/api/v1/urls. A successful response will return an array of url objects.
